/* === SONGS_BROWSER.JS - Motor de Búsqueda Premium Restaurado 💎 === */
...
window.fetchUnifiedData = async function(query = "", append = false) {
    ...
    if (!append) {
        ...
        grid.innerHTML = '<div style="width:100%; text-align:center; padding:50px; color:#00ffff; font-size:1.5rem; font-weight:bold; text-shadow: 0 0 15px #00ffff; animation: pulse 1.5s infinite;">Sincronizando Galería Global... ⏳</div>';
    } else {
        const btnLoad = document.getElementById('btn-load-more');
        if (btnLoad) btnLoad.innerText = "DESCARGANDO MÁS MAPAS... 🌐";
    }
...
window.renderUnifiedGrid = function() {
    ...
    filtered.forEach((song, index) => {
        // --- MOTOR DE COLORES DE MODO ---
        let modeColor = '#00ffff'; 
        let modeIcon = '☁️'; 
        let modeLabel = 'COMUNIDAD';
        ...
        card.style.cssText = `position: relative; height: 180px; border-radius: 12px; overflow: hidden; cursor: pointer; border: 2px solid ${modeColor}44; background: #111; transition: all 0.3s cubic-bezier(0.17, 0.89, 0.32, 1.28); animation: fadeIn 0.4s ease-out forwards; animation-delay: ${index * 0.03}s; opacity: 0;`;
        
        // Badges con Brillo Neón
        let badgesHTML = `<div class="diff-badge" style="border: 1px solid ${starCol}; color: ${starCol}; background: rgba(0,0,0,0.7); box-shadow: 0 0 10px ${starCol}44;">⭐ ${parseFloat(stars).toFixed(1)}</div>`;
        ...
        badgesHTML += `<div class="diff-badge" style="margin-left:auto; border: 1px solid ${modeColor}; color: ${modeColor}; background: rgba(0,0,0,0.8); box-shadow: 0 0 10px ${modeColor}66;">${modeIcon} ${modeLabel}</div>`;
        ...
        card.innerHTML = `
            ...
            <div class="song-info" style="position: absolute; bottom: 0; left: 0; width: 100%; padding: 15px; background: linear-gradient(to top, rgba(0,0,0,0.95) 40%, rgba(0,0,0,0.7), transparent); z-index: 2;">
                ...
                <div class="song-author" style="font-size: 0.9rem; color: #bbb; font-weight: bold; margin-bottom: 12px; opacity: 0.8;">${song.artist}</div>
                <div style="display:flex; gap:6px; flex-wrap:wrap; align-items:center;">
                    ${badgesHTML}
                </div>
            </div>
            <div class="card-overlay" style="position:absolute; top:0; left:0; width:100%; height:100%; background: linear-gradient(45deg, ${modeColor}22, transparent); opacity: 0; transition: 0.3s; z-index: 1;"></div>
        `;
        ...
        card.onmouseenter = () => {
            ...
            card.style.boxShadow = `0 10px 30px rgba(0,0,0,0.5), 0 0 20px ${modeColor}66`;
            card.querySelector('.song-bg').style.opacity = '0.8';
