/* === SONGS_BROWSER.JS - FIX DE EMOJIS, FILTROS Y ORDEN === */
...
window.fetchUnifiedData = async function(query = "", append = false) {
    ...
    // 1. CARGAR DE FIREBASE (COMUNIDAD)
    if (!append) {
        try {
            if(window.db) {
    ...
    // El orden: Siempre Firebase primero, luego Osu
...
window.renderUnifiedGrid = function() {
    ...
    let filtered = baseList.filter(song => {
        ...
        // FILTRO DE TIPO ESTRICTO
        if (f.type !== 'all' && f.type !== 'recent') {
            if (f.type === 'com') { if (song.isOsu) return false; }
            else if (f.type === 'charted') { if (song.isOsu || countTotalNotes(song.raw) === 0) return false; }
            else if (f.type === 'mechanics') { if (song.isOsu || !song.raw || !song.raw.mechanics || song.raw.mechanics.length === 0) return false; }
            else if (f.type.startsWith('osu_')) {
                const target = f.type.replace('osu_', '');
                if (song.originalMode !== target) return false; // Filtra estrictamente Standard, Taiko, etc.
            }
        }
        ...
        if (f.key !== 'all') {
            ...
            if (song.originalMode !== 'mania') return false; // Solo Mania tiene teclas
    ...
    filtered.forEach(song => {
        ...
        let oMode = song.originalMode || 'mania';
        let mIcon = oMode === 'standard' ? '🎯' : (oMode === 'taiko' ? '🥁' : (oMode === 'catch' ? '🍎' : '🎹'));
        let mColor = oMode === 'standard' ? '#ff44b9' : (oMode === 'taiko' ? '#f95555' : (oMode === 'catch' ? '#44b9ff' : '#ff66aa'));

        // 🚨 BADGES ALINEADOS EXACTAMENTE COMO QUERÍAS
        let topBadge = !song.isOsu ? `<div class="diff-badge" style="border-color: var(--blue); color: var(--blue);">☁️ COMMUNITY</div>` : `<div class="diff-badge" style="border-color: #ff66aa; color: #ff66aa;">🌸 OSU!</div>`;
        let modeBadge = `<div class="diff-badge" style="border-color: ${mColor}; color: ${mColor};">${mIcon} ${oMode.toUpperCase()}</div>`;
        let maniaKeys = oMode === 'mania' ? (song.keysAvailable || []).filter(k => k > 1).map(k => `<div class="diff-badge" style="border: 1px solid var(--blue); color: var(--blue);">${k}K</div>`).join('') : "";
        ...
        let mechBadge = (!song.isOsu && song.raw && song.raw.mechanics && song.raw.mechanics.length > 0) ? `<div class="diff-badge" style="border-color:#00ffff; color:#00ffff; font-weight:900;">⚙️ FX</div>` : ``;

        // 🚨 LOS BADGES AHORA SALEN EN ORDEN: SOURCE -> MODE -> KEYS -> CHARTED
        card.innerHTML = `
            ...
            <div class="song-info" style="position:relative; z-index:2; padding:15px; display:flex; flex-direction:column; justify-content:flex-end; height:100%;">
                ...
                <div style="display:flex; gap:8px; flex-wrap:wrap; align-items:center; margin-top:10px;">
                    ...
                    ${topBadge} ${modeBadge} ${chartedBadge} ${mechBadge} ${maniaKeys}
                </div>
            </div>`;
