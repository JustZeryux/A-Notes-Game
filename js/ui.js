...
// Actualizar la interfaz con los datos del usuario (Stats, XP, Nivel)
function updUI() {
    // Datos básicos
    ...
    // Cálculo de XP requerida (Curva exponencial suave)
    ...
    document.getElementById('p-global-rank').innerText = "#--"; // Se actualiza al abrir ranking
    
    // Avatar
    if(user.avatarData) { 
        document.getElementById('m-av').style.backgroundImage = `url(${user.avatarData})`; 
    ...
    // Cargar valores en el menú de ajustes
    ...
    // Mostrar/Ocultar gestión de cuenta según tipo de login
...
function openFriends() {
    ...
    // Escuchar cambios en la lista de amigos
        ...
        if(data && data.friends && data.friends.length > 0) {
            ...
            data.friends.forEach(f => {
                ...
                db.collection("users").doc(f).get().then(fDoc => {
                    ...
                    // Calcular estado online (si se vio en los últimos 2 mins)
                    ...
                    d.innerHTML = `
                        <div style="display:flex;align-items:center; pointer-events:none;">
                            ...
                            <div class="f-row-av" style="${avStyle}"></div>
                            <span class="friend-row-name">${f}</span>
                        </div>`;
                    friL.appendChild(d);
                });
            });
        } else { 
            friL.innerHTML = '<div style="color:#666; padding:20px;">No tienes amigos aún. ¡Agrega a alguien!</div>'; 
        }
    });
    openModal('friends');
}
...
function showFriendProfile(fName, fData, isOnline) {
    ...
    if(isOnline) {
        ...
        statusTxt.style.color = "var(--good)";
...
function initFloatChatListener(target) {
    ...
    db.collection("chats").doc(room).collection("messages")
        ...
        .onSnapshot(snapshot => {
            ...
            // Mostrar mensajes (invertir porque Firestore los da en orden cronológico, y queremos el último abajo)
...
function notifyInteractive(id, title, html, duration) {
    ...
    card.innerHTML = `
        <div class="notify-content">
            ...
            <div class="notify-body">${html}</div>
        </div>
        <div class="notify-progress" style="transition-duration:${duration}ms"></div>`;
...
// --- RENDERIZADO DEL MENÚ DE CANCIONES (GLOBAL) ---
...
function renderMenu(filter="") {
    ...
    // Escuchar canciones ordenadas por fecha
    globalSongsListener = db.collection("globalSongs").orderBy("createdAt", "desc").limit(50)
        .onSnapshot(snapshot => {
            ...
            snapshot.forEach(doc => {
                ...
                // Imagen de fondo (URL o degradado)
                ...
                // Verificar si el usuario tiene puntuación en esta canción
                let scoreTag = '';
                ...
                c.onclick = () => { 
                    ...
                    openModal('diff'); 
                    // Ocultar botón de lobby si estamos en modo solo
...
// === AUTO COMPLETAR TÍTULO AL SELECCIONAR ARCHIVO ===
function autoFillTitle(input) {
    if(input.files[0]) {
        let name = input.files[0].name.replace(/\.[^/.]+$/, ""); // Quitar extensión
        name = name.replace(/[_-]/g, " "); // Reemplazar guiones con espacios
        document.getElementById('up-title').value = name;
        document.getElementById('upload-status').innerText = ""; 
    }
}

// === HELPER PARA SUBIR A UPLOADCARE ===
function uploadFileToUC(file) {
    return new Promise((resolve, reject) => {
        // Usa la librería de Uploadcare cargada en el HTML
...
// === SUBIDA GLOBAL DE CANCIONES (CON UPLOADCARE) ===
async function startGlobalUpload() {
    if(!db) return notify("Error: Base de datos no conectada", "error");
    ...
    // Límite local para no saturar Uploadcare demo (20MB)
    if(audioFile.size > 20 * 1024 * 1024) return notify("El audio es muy pesado (Max 20MB)", "error");

    btn.disabled = true;
    btn.innerText = "PROCESANDO...";
    status.innerText = "Subiendo audio a la nube...";
    ...
    try {
        ...
        console.error(e);
        notify("Error al subir: " + e, "error");
        status.innerText = "Falló la subida.";
    } finally {
        btn.disabled = false;
...
// Cargar Hit Sound personalizado (Local)
...
function openModal(id){ 
    ...
    if(id==='upload') {
         document.getElementById('upload-status').innerText = "";
         // No limpiamos inputs aquí para que si cierra por error no pierda datos
    }
    
    // Lógica visual del modal de dificultad
...
function saveSettings(){ 
    ...
    // Llamar a save() en auth.js para persistir
...
function renderLaneConfig(k){ 
    ...
    for(let i=0; i<k; i++){ 
        ...
        d.innerHTML = `
            <div class="key-bind ${remapIdx===i && remapMode===k?'listening':''}" onclick="remapKey(${k},${i})">
                ${l.k.toUpperCase()}
            </div>
            <div class="shape-indicator" onclick="cycleShape(${k},${i})">
                <svg class="shape-svg-icon" viewBox="0 0 100 100"><path d="${shapePath}"/></svg>
            </div>
...
function updateLaneColor(k,i,v){ 
    cfg.modes[k][i].c = v; 
}
