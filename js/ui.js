    ...
    allModes.forEach(k => {
        ...
        if (song.keysAvailable.includes(k)) {
            ...
            btn.onclick = () => {
                closeModal('diff');
                asegurarModo(k); 
                window.saveToRecents(song); // <--- AÑADIR ESTA LÍNEA
...
window.showUserProfile = async function(targetName) {
    ...
    window.db.collection("leaderboard").orderBy("pp", "desc").limit(50).get().then(async snap => {
        ...
        // 2. Descargar TODOS los avatares/niveles al mismo tiempo (Esto elimina la carga de 1 en 1)
        ...
        topPlayers.forEach((d, i) => {
            ...
            card.onclick = () => { closeModal('rank'); showUserProfile(d.name); };
...
// ñ LISTA DE AMIGOS AAA (CON ESTADO ONLINE) ==
window.openLeaderboard = function() {
    openModal('rank');
    const list = document.getElementById('rank-list');
    if(!list) return;
...
window.openFriends = function() {
    ...
    window.db.collection("users").doc(window.user.name).onSnapshot(async doc => {
        ...
        if(reqList) {
            reqList.innerHTML = '';
            if(data.requests && data.requests.length > 0) {
                data.requests.forEach(reqName => { /* (Tu código de solicitudes igual) */ });
            }
        }

        // Dibujar Amigos y verificar conexión en paralelo
        if(friList) {
            friList.innerHTML = '';
            if(data.friends && data.friends.length > 0) {
                ...
                data.friends.forEach((fName, i) => {
                    ...
                    if(fDoc && fDoc.exists) {
                        ...
                        lvl = fd.lvl || 1;
                        pp = fd.pp || 0;
                        // Si la última conexión fue hace menos de 5 minutos (300,000 ms), está Online
